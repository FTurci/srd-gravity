...
FixStyle(srdf,FixSRDF)

#else

#ifndef LMP_FIX_SRDF_H
#define LMP_FIX_SRDF_H

#include "fix.h"

namespace LAMMPS_NS {

class FixSRDF : public Fix {
 public:
  FixSRDF(class LAMMPS *, int, char **);
  ~FixSRDF();
  int setmask();
  void init();
  void setup(int);
 ...
 private:
  ...
  double srdlo[3],srdhi[3];                  // SRDFs must stay inside
  double srdlo_reneigh[3],srdhi_reneigh[3];  // SRDFs trigger a reneigh
  ...
  int nmax;               // max number of SRDF particles


  //Francesco Turci
  double external_acceleration;

  // bins for SRDF velocity remap, shifting and communication
  ...
  struct BinAve {
    ...
    int n;               // # of SRDF particles in bin
    double xctr[3];      // center point of bin, only used for triclinic
    double vsum[3];      // sum of v components for SRDF particles in bin
  ...
  int maxbin1;
  int *binhead;          // 1st SRDF particle in each bin
  int *binnext;          // next SRDF particle in same bin
  int maxbuf;
  double *sbuf1,*sbuf2;  // buffers for send/recv of velocity bin data
  double *rbuf1,*rbuf2;

  // bins and stencil for collision searching for SRDFs & big particles
  ...
  int *binsrd;           // which bin each SRDF particle is in
...
This is because the SRDF collisions will impart torque to the solute
particles.

E: Cannot change timestep once fix srd is setup

This is because various SRDF properties depend on the timestep
size.
...
W: Fix SRDF walls overlap but fix srd overlap not set

You likely want to set this in your input script.

E: Using fix srd with inconsistent fix deform remap option

When shearing the box in an SRDF simulation, the remap v option for fix
deform needs to be used.

W: Using fix srd with box deformation but no SRDF thermostat

The deformation will heat the SRDF particles so this can
be dangerous.

W: Fix srd SRDF moves may trigger frequent reneighboring

This is because the SRDF particles may move long distances.

E: Fix SRDF: bad search bin assignment

Something has gone wrong in your SRDF model; try using more
conservative settings.

E: Fix SRDF: bad stencil bin for big particle

Something has gone wrong in your SRDF model; try using more
conservative settings.

E: Fix SRDF: too many big particles in bin

Reset the ATOMPERBIN parameter at the top of fix_srd.cpp
to a larger value, and re-compile the code.

E: Fix SRDF: too many walls in bin

This should not happen unless your system has been setup incorrectly.

E: Fix SRDF: bad bin assignment for SRDF advection

Something has gone wrong in your SRDF model; try using more
conservative settings.

E: SRDF particle %d started inside big particle %d on step %ld bounce %d

See the inside keyword if you want this message to be an error vs
warning.

W: SRDF particle %d started inside big particle %d on step %ld bounce %d

See the inside keyword if you want this message to be an error vs
warning.

E: SRDF particle %d started inside wall %d on step %ld bounce %d

See the inside keyword if you want this message to be an error vs
warning.

W: SRDF particle %d started inside wall %d on step %ld bounce %d
...
E: Fix srd requires SRDF particles all have same mass

Self-explanatory.

E: Fewer SRDF bins than processors in some dimension

This is not allowed.  Make your SRDF bin size smaller.

E: SRDF bins for fix srd are not cubic enough

The bin shape is not within tolerance of cubic.  See the cubic
keyword if you want this message to be an error vs warning.

W: SRDF bins for fix srd are not cubic enough

The bin shape is not within tolerance of cubic.  See the cubic
keyword if you want this message to be an error vs warning.

E: SRDF bin size for fix srd differs from user request

Fix SRDF had to adjust the bin size to fit the simulation box.  See the
cubic keyword if you want this message to be an error vs warning.

W: SRDF bin size for fix srd differs from user request

Fix SRDF had to adjust the bin size to fit the simulation box.  See the
cubic keyword if you want this message to be an error vs warning.

E: Fix srd lamda must be >= 0.6 of SRDF grid size

This is a requirement for accuracy reasons.

W: SRDF bin shifting turned on due to small lamda
...
W: Fix srd viscosity < 0.0 due to low SRDF density
