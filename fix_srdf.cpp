...
enum{BIG_MOVE,SRDF_MOVE,SRDF_ROTATE};
...
//#define SRDF_DEBUG 1
//#define SRDF_DEBUG_ATOMID 58
//#define SRDF_DEBUG_TIMESTEP 449

/* ---------------------------------------------------------------------- */

FixSRDF::FixSRDF(LAMMPS *lmp, int narg, char **arg) : Fix(lmp, narg, arg),
  ...
  binsrd(NULL), stencil(NULL)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"lamda") == 0) {
      ...
      iarg += 2;
    } else if (strcmp(arg[iarg],"acceleration") == 0) {
      printf("iarg %d narg %d\n", iarg, narg);
      if (iarg+2 > narg) error->all(FLERR,"Illegal fix srd command");
      external_acceleration = force->numeric(FLERR,arg[iarg+1]);
      iarg += 2;
      printf("::::: The imposed acceleration is %g\n",external_acceleration );
    } 
...
void FixSRDF::init()
{
  ...
  // wallexist = 1 if SRDF wall(s) are defined
  ...
  for (int m = 0; m < modify->nfix; m++) {
    if (strcmp(modify->fix[m]->style,"wall/srd") == 0) {
      ...
      if (wallfix->overlap && overlap == 0 && me == 0)
        error->warning(FLERR,
                       "Fix SRDF walls overlap but fix srd overlap not set");
    }
  }
  ...
  if (deformflag && tstat == 0 && me == 0)
    error->warning(FLERR,
                   "Using fix srd with box deformation but no SRDF thermostat");
  ...
  // limit initial SRDF velocities if necessary
  ...
  if (me == 0) {
    if (screen)
      fprintf(screen,"  # of rescaled SRDF velocities = %d\n",all);
    if (logfile)
      fprintf(logfile,"  # of rescaled SRDF velocities = %d\n",all);
  }
...
void FixSRDF::setup(int vflag)
{
  setup_bounds();

  if (dist_srd_reneigh < nevery*dt_big*vmax && me == 0)
    error->warning(FLERR,
                   "Fix srd SRDF moves may trigger frequent reneighboring");
  ...
  // perform first bining of SRDF and big particles and walls
  // set reneighflag to turn off SRDF rotation
  // don't do SRDF rotation in setup, only during timestepping
...
/* ----------------------------------------------------------------------
   assign SRDF particles to bins
   assign big particles to all bins they overlap
------------------------------------------------------------------------- */

void FixSRDF::pre_neighbor()
{
  int i,j,m,ix,iy,iz,jx,jy,jz,ibin,jbin,lo,hi;
  double rsq,cutbinsq;

  // grow SRDF per-atom bin arrays if necessary
  ...
  if (bigexist || wallexist) {
    ...
    // Francesco Turci: scale the external acceleration by the number of particles
    ...
    while (i < nall) {
      if (mask[i] & biggroupbit) {
        ...
        if (ix < 0 || ix >= nbin2x || iy < 0 || iy >= nbin2y ||
            iz < 0 || iz >= nbin2z)
          error->one(FLERR,"Fix SRDF: bad search bin assignment");
        ...
        for (j = 0; j < nstencil; j++) {
          ...
          if (jx < 0 || jx >= nbin2x || jy < 0 || jy >= nbin2y ||
            ...
            error->one(FLERR,"Fix SRDF: bad stencil bin for big particle");
          }
          rsq = point_bin_distance(x[i],jx,jy,jz);
          if (rsq < cutbinsq) {
            jbin = ibin + stencil[j][3];
            if (nbinbig[jbin] == ATOMPERBIN)
              error->one(FLERR,"Fix SRDF: too many big particles in bin");
      ...
      if (dim == 0) {
        if (side == 0) {
          ...
          if (hi >= nbin2x) error->all(FLERR,
                                       "Fix SRDF: bad search bin assignment");
          lo = 0;
        } else {
          lo = static_cast<int> ((xwall[m]-delta-xblo2)*bininv2x);
          if (lo >= nbin2x) continue;
          if (lo < 0) error->all(FLERR,"Fix SRDF: bad search bin assignment");
          hi = nbin2x-1;
        }
          ...
          for (iy = 0; iy < nbin2y; iy++)
            for (iz = 0; iz < nbin2z; iz++) {
              ...
              if (nbinbig[ibin] == ATOMPERBIN)
                error->all(FLERR,"Fix SRDF: too many walls in bin");
  ...
  // rotate SRDF velocities on SRDF timestep
  // done now since all SRDFs are currently inside my sub-domain

  if (reneighflag == SRDF_ROTATE) reset_velocities();

  // log stats if reneighboring occurred b/c SRDFs moved too far

  if (reneighflag == SRDF_MOVE) reneighcount++;
  reneighflag = BIG_MOVE;
}

/* ----------------------------------------------------------------------
   advect SRDF particles and detect collisions between SRDF and BIG particles
   when collision occurs, change x,v of SRDF, force,torque of BIG particle
------------------------------------------------------------------------- */

void FixSRDF::post_force(int vflag)
{
  int i,m,ix,iy,iz;
  ...
  if (bigexist || wallexist) {
    for (i = 0; i < nlocal; i++)
      if (mask[i] & groupbit) {
        x[i][0] += dt_big*v[i][0];
        x[i][1] += dt_big*v[i][1];
        x[i][2] += dt_big*v[i][2]+0.5*external_acceleration*dt_big*dt_big;
        
        v[i][2] += external_acceleration*dt_big;
        ...
        if (ix < 0 || ix >= nbin2x || iy < 0 || iy >= nbin2y ||
            iz < 0 || iz >= nbin2z) {
          if (screen) {
            fprintf(screen,"SRDF particle " TAGINT_FORMAT
          ...
          error->one(FLERR,"Fix SRDF: bad bin assignment for SRDF advection");
        }
      }
  ...
  // detect collision of SRDFs with BIG particles or walls
  ...
  // if any SRDF particle has moved too far, trigger reneigh on next step
  ...
  // if next timestep is SRDF timestep, trigger reneigh

  if ((update->ntimestep+1) % nevery == 0) {
    next_reneighbor = update->ntimestep + 1;
    reneighflag = SRDF_ROTATE;
  }
}

/* ----------------------------------------------------------------------
   reset SRDF velocities
   may perform random shifting by up to 1/2 bin in each dimension
   called at pre-neighbor stage when all SRDFs are now inside my sub-domain
   if tstat, then thermostat SRDF particles as well, including streaming effects
------------------------------------------------------------------------- */

void FixSRDF::reset_velocities()
{
  ...
  // binhead = 1st SRDF particle in each bin
  ...
  double *h_ratelo = domain->h_ratelo;
    ...
    //FT: Thermostat according to Padding and Louis PHYSICAL REVIEW E 74, 031402  2006 

  double Tmeas=0, Nfree=0;
  ...
  if (tstat) {
    for (i = 0; i < nbins; i++){
      ...
      scale = sqrt(temperature_srd/Tmeas);
...
void FixSRDF::vbin_comm(int ishift)
{
...
void FixSRDF::vbin_pack(BinAve *vbin, int n, int *list, double *buf)
{
  int j;
  int m = 0;
  for (int i = 0; i < n; i++) {
...
void FixSRDF::vbin_unpack(double *buf, BinAve *vbin, int n, int *list)
{
  int j;
  int m = 0;
  for (int i = 0; i < n; i++) {
...
void FixSRDF::xbin_comm(int ishift, int nval)
{
  BinComm *bcomm1,*bcomm2;
  MPI_Request request1,request2;
...
void FixSRDF::xbin_pack(BinAve *vbin, int n, int *list, double *buf, int nval)
{
  int j, k;
  int m = 0;
  for (int i = 0; i < n; i++) {
...
void FixSRDF::xbin_unpack(double *buf, BinAve *vbin, int n, int *list, int nval)
{
  int j, k;
  int m = 0;
  for (int i = 0; i < n; i++) {
...
/* ----------------------------------------------------------------------
   detect all collisions between SRDF and BIG particles or WALLS
   assume SRDF can be inside at most one BIG particle or WALL at a time
   unoverlap SRDFs for each collision
------------------------------------------------------------------------- */

void FixSRDF::collisions_single()
{
  ...
  // outer loop over SRDF particles
  // inner loop over BIG particles or WALLS that overlap SRDF particle bin
  // if overlap between SRDF and BIG particle or wall:
  //   for exact, compute collision pt in time
  //   for inexact, push SRDF to surf of BIG particle or WALL
  // update x,v of SRDF and f,torque on BIG particle
  // re-bin SRDF particle after collision
  // iterate until the SRDF particle has no overlaps with BIG particles or WALLS
    ...
    while (collide_flag) {
      ...
      for (m = 0; m < nbig; m++) {
        ...
        if (inside) {
...
#ifdef SRDF_DEBUG
          if (update->ntimestep == SRDF_DEBUG_TIMESTEP &&
              atom->tag[i] == SRDF_DEBUG_ATOMID)
          ...
          if (t_remain > dt) {
            ...
            if (insideflag == INSIDE_ERROR || insideflag == INSIDE_WARN) {
              ...
              if (type != WALL) {
                sprintf(str,
                        "SRDF particle " TAGINT_FORMAT " started "
		...
		error->warning(FLERR,str);
          ...
          // update BIG particle and WALL and SRDF
...
/* ----------------------------------------------------------------------
   detect all collisions between SRDF and big particles
   an SRDF can be inside more than one big particle at a time
   requires finding which big particle SRDF collided with first
   unoverlap SRDFs for each collision
------------------------------------------------------------------------- */

void FixSRDF::collisions_multi()
{
  ...
  // outer loop over SRDF particles
  // inner loop over BIG particles or WALLS that overlap SRDF particle bin
  // loop over all BIG and WALLS to find which one SRDF collided with first
  // if overlap between SRDF and BIG particle or wall:
...
/* ----------------------------------------------------------------------
   check if SRDF particle S is inside spherical big particle B
------------------------------------------------------------------------- */

int FixSRDF::inside_sphere(double *xs, double *xb, Big *big)
{
  double dx,dy,dz;
...
/* ----------------------------------------------------------------------
   check if SRDF particle S is inside ellipsoidal big particle B
------------------------------------------------------------------------- */

int FixSRDF::inside_ellipsoid(double *xs, double *xb, Big *big)
{
  double x,y,z;
...
/* ----------------------------------------------------------------------
   check if SRDF particle S is inside line big particle B
...
int FixSRDF::inside_line(double *xs, double *xb, double *vs, double *vb,
...
/* ----------------------------------------------------------------------
   check if SRDF particle S is inside triangle big particle B
   collision only possible if:
...
int FixSRDF::inside_tri(double *xs, double *xb, double *vs, double *vb,
                       Big *big, double dt_step)
{
...
/* ----------------------------------------------------------------------
   check if SRDF particle S is inside wall IWALL
------------------------------------------------------------------------- */

int FixSRDF::inside_wall(double *xs, int iwall)
{
...
/* ----------------------------------------------------------------------
   collision of SRDF particle S with surface of spherical big particle B
   exact because compute time of collision
   dt = time previous to now at which collision occurs
   xscoll = collision pt = position of SRDF at time of collision
   xbcoll = position of big particle at time of collision
   norm = surface normal of collision pt at time of collision
------------------------------------------------------------------------- */

double FixSRDF::collision_sphere_exact(double *xs, double *xb,
...
/* ----------------------------------------------------------------------
   collision of SRDF particle S with surface of spherical big particle B
   inexact because just push SRDF to surface of big particle at end of step
...
void FixSRDF::collision_sphere_inexact(double *xs, double *xb,
                                      Big *big,
                                      double *xscoll, double *xbcoll,
                                      double *norm)
{
...
/* ----------------------------------------------------------------------
   collision of SRDF particle S with surface of ellipsoidal big particle B
   exact because compute time of collision
   dt = time previous to now at which collision occurs
   xscoll = collision pt = position of SRDF at time of collision
   xbcoll = position of big particle at time of collision
...
double FixSRDF::collision_ellipsoid_exact(double *xs, double *xb,
                                         double *vs, double *vb, Big *big,
                                         double *xscoll, double *xbcoll,
                                         double *norm)
{
...
void FixSRDF::collision_ellipsoid_inexact(double *xs, double *xb,
                                         Big *big,
                                         double *xscoll, double *xbcoll,
                                         double *norm)
{
...
/* ----------------------------------------------------------------------
   collision of SRDF particle S with surface of line big particle B
   exact because compute time of collision
   dt = time previous to now at which collision occurs
   xscoll = collision pt = position of SRDF at time of collision
   xbcoll = position of big particle at time of collision
...
double FixSRDF::collision_line_exact(double *xs, double *xb,
                                    double *vs, double *vb, Big *big,
                                    double dt_step,
                                    double *xscoll, double *xbcoll,
                                    double *norm)
...
/* ----------------------------------------------------------------------
   collision of SRDF particle S with surface of triangle big particle B
   exact because compute time of collision
   dt = time previous to now at which collision occurs
   xscoll = collision pt = position of SRDF at time of collision
   xbcoll = position of big particle at time of collision
...
double FixSRDF::collision_tri_exact(double *xs, double *xb,
                                   double *vs, double *vb, Big *big,
                                   double dt_step,
                                   double *xscoll, double *xbcoll,
                                   double *norm)
...
/* ----------------------------------------------------------------------
   collision of SRDF particle S with wall IWALL
   exact because compute time of collision
   dt = time previous to now at which collision occurs
   xscoll = collision pt = position of SRDF at time of collision
...
double FixSRDF::collision_wall_exact(double *xs, int iwall, double *vs,
                                     double *xscoll, double *xbcoll,
                                     double *norm)
{
  int dim = wallwhich[iwall] / 2;
...
/* ----------------------------------------------------------------------
   collision of SRDF particle S with wall IWALL
   inexact because just push SRDF to surface of wall at end of step
   time of collision = end of step
   xscoll = collision pt = position of SRDF at time of collision
   xbcoll = position of wall at time of collision
   norm = surface normal of collision pt at time of collision
...
void FixSRDF::collision_wall_inexact(double *xs, int iwall, double *xscoll,
...
/* ----------------------------------------------------------------------
   ...
   vs = velocity of SRDF, vb = velocity of BIG
   ...
   v of BIG particle in direction of surf normal is added to v of SRDF
   includes component due to rotation of BIG
   return vsnew of SRDF
------------------------------------------------------------------------- */

void FixSRDF::slip(double *vs, double *vb, double *xb, Big *big,
...
/* ----------------------------------------------------------------------
   ...
   v of WALL in direction of surf normal is added to v of SRDF
   return vsnew of SRDF
------------------------------------------------------------------------- */

void FixSRDF::slip_wall(double *vs, int iwall, double *norm, double *vsnew)
{
...
/* ----------------------------------------------------------------------
   ...
   xsurf = collision pt on surf of BIG
   norm = unit normal from surface of BIG at collision pt
   v of collision pt is added to v of SRDF
   includes component due to rotation of BIG
   return vsnew of SRDF
------------------------------------------------------------------------- */

void FixSRDF::noslip(double *vs, double *vb, double *xb, Big *big, int iwall,
                    double *xsurf, double *norm, double *vsnew)
{
  double vs_dot_n,scale,r1,r2,vnmag,vtmag1,vtmag2;
  double tangent1[3],tangent2[3];
...
/* ----------------------------------------------------------------------
   ...
   force on BIG particle = -dp/dt of SRDF particle
   torque on BIG particle = r cross (-dp/dt)
------------------------------------------------------------------------- */

void FixSRDF::force_torque(double *vsold, double *vsnew,
                          double *xs, double *xb,
                          double *fb, double *tb)
{
...
/* ----------------------------------------------------------------------
   ...
   force on WALL = -dp/dt of SRDF particle
------------------------------------------------------------------------- */

void FixSRDF::force_wall(double *vsold, double *vsnew, int iwall)
...
/* ----------------------------------------------------------------------
   update SRDF particle position & velocity & search bin assignment
   check if SRDF moved outside of valid region
   if so, may overlap off-processor BIG particle
------------------------------------------------------------------------- */

int FixSRDF::update_srd(int i, double dt, double *xscoll, double *vsnew,
                       double *xs, double *vs)
{
  int ix,iy,iz;
...
/* ----------------------------------------------------------------------
   setup all SRDF parameters with big particles
------------------------------------------------------------------------- */

void FixSRDF::parameterize()
{
  // timesteps
  ...
  // mass of SRDF particles, require monodispersity
  ...
  if (flagall)
    error->all(FLERR,"Fix srd requires SRDF particles all have same mass");

  // set temperature and lamda of SRDF particles from each other
  ...
  // vmax = maximum velocity of an SRDF particle
  // dmax = maximum distance an SRDF can move = 4*lamda = vmax * dt_srd
  ...
  // create grid for binning/rotating SRDF particles from gridsrd
  ...
  // srd_per_grid = # of SRDF particles per SRDF grid cell
  ...
  // kinematic viscosity of SRDF fluid
  ...
  // print SRDF parameters

  if (me == 0) {
    if (screen) {
      fprintf(screen,"SRDF info:\n");
      fprintf(screen,
              "  SRDF/big particles = " BIGINT_FORMAT " " BIGINT_FORMAT "\n",
              nsrd,mbig);
      fprintf(screen,"  big particle diameter max/min = %g %g\n",
              maxbigdiam,minbigdiam);
      fprintf(screen,"  SRDF temperature & lamda = %g %g\n",
              temperature_srd,lamda);
      fprintf(screen,"  SRDF max distance & max velocity = %g %g\n",dmax,vmax);
      fprintf(screen,"  SRDF grid counts: %d %d %d\n",nbin1x,nbin1y,nbin1z);
      fprintf(screen,"  SRDF grid size: request, actual (xyz) = %g, %g %g %g\n",
              gridsrd,binsize3x,binsize3y,binsize3z);
      fprintf(screen,"  SRDF per actual grid cell = %g\n",srd_per_cell);
      fprintf(screen,"  SRDF viscosity = %g\n",viscosity);
      fprintf(screen,"  big/SRDF mass density ratio = %g\n",mdratio);
    }
    if (logfile) {
      fprintf(logfile,"SRDF info:\n");
      fprintf(logfile,
              "  SRDF/big particles = " BIGINT_FORMAT " " BIGINT_FORMAT "\n",
              nsrd,mbig);
      fprintf(logfile,"  big particle diameter max/min = %g %g\n",
              maxbigdiam,minbigdiam);
      fprintf(logfile,"  SRDF temperature & lamda = %g %g\n",
              temperature_srd,lamda);
      fprintf(logfile,"  SRDF max distance & max velocity = %g %g\n",dmax,vmax);
      fprintf(logfile,"  SRDF grid counts: %d %d %d\n",nbin1x,nbin1y,nbin1z);
      fprintf(logfile,"  SRDF grid size: request, actual (xyz) = %g, %g %g %g\n",
              gridsrd,binsize3x,binsize3y,binsize3z);
      fprintf(logfile,"  SRDF per actual grid cell = %g\n",srd_per_cell);
      fprintf(logfile,"  SRDF viscosity = %g\n",viscosity);
      fprintf(logfile,"  big/SRDF mass density ratio = %g\n",mdratio);
    }
  }

  // error if less than 1 SRDF bin per processor in some dim

  if (nbin1x < comm->procgrid[0] || nbin1y < comm->procgrid[1] ||
      nbin1z < comm->procgrid[2])
    error->all(FLERR,"Fewer SRDF bins than processors in some dimension");

  // check if SRDF bins are within tolerance for shape and size
  ...
  if (tolflag) {
    if (cubicflag == CUBIC_ERROR)
      error->all(FLERR,"SRDF bins for fix srd are not cubic enough");
    if (me == 0)
      error->warning(FLERR,"SRDF bins for fix srd are not cubic enough");
  }
  ...
  if (tolflag) {
    if (cubicflag == CUBIC_ERROR)
      error->all(FLERR,"SRDF bin size for fix srd differs from user request");
    if (me == 0)
      error->warning(FLERR,
                     "SRDF bin size for fix srd differs from user request");
  }

  // error if lamda < 0.6 of SRDF grid size and no shifting allowed
  ...
  if (lamda < 0.6*maxgridsrd && shiftuser == SHIFT_NO)
    error->all(FLERR,"Fix srd lamda must be >= 0.6 of SRDF grid size");
  else if (lamda < 0.6*maxgridsrd && shiftuser == SHIFT_POSSIBLE) {
    shiftflag = 1;
    if (me == 0)
      error->warning(FLERR,"SRDF bin shifting turned on due to small lamda");
  ...
  if (viscosity < 0.0 && me == 0)
    error->warning(FLERR,"Fix srd viscosity < 0.0 due to low SRDF density");
...
/* ----------------------------------------------------------------------
   set bounds for big and SRDF particle movement
   called at setup() and when box size changes (but not shape)
------------------------------------------------------------------------- */

void FixSRDF::setup_bounds()
{
  ...
  // dist_srd = distance from SD at which SRDF could collide with unknown CO,
  //   used to error check bounds of SRDF movement after collisions via srdlo/hi
  // dist_srd_reneigh = distance from SD at which an SRDF should trigger
  //   a reneigh, b/c next SRDF move might overlap with unknown CO,
  //   used for SRDF triggering of reneighboring via srdlo/hi_reneigh
  // onemove = max distance an SRDF can move in one step
  ...
  // lo/hi = bbox on this proc which SRDF particles must stay inside
  // lo/hi reneigh = bbox on this proc outside of which SRDFs trigger a reneigh
...
/* ----------------------------------------------------------------------
   setup bins used for binning SRDF particles for velocity reset
...
void FixSRDF::setup_velocity_bins()
{
...
void FixSRDF::setup_velocity_shift(int ishift, int dynamic)
{
...
/* ----------------------------------------------------------------------
   setup bins used for big and SRDF particle searching
...
void FixSRDF::setup_search_bins()
{
...
void FixSRDF::setup_search_stencil()
{
...
double FixSRDF::point_bin_distance(double *x, int i, int j, int k)
{
...
double FixSRDF::bin_bin_distance(int i, int j, int k)
{
  double delx,dely,delz;
...
int FixSRDF::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void FixSRDF::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
double FixSRDF::compute_vector(int n)
{
...
void FixSRDF::velocity_stats(int groupnum)
{
...
double FixSRDF::newton_raphson(double t1, double t2)
{
...
void FixSRDF::lineside(double t, double &f, double &df)
{
  double p[2],c[2];
...
void FixSRDF::triside(double t, double &f, double &df)
{
  double p[2],c[2];
...
double FixSRDF::distance(int i, int j)
{
...
void FixSRDF::print_collision(int i, int j, int ibounce,
                              ...
                              int type)
{
  ...
  if (type != WALL) {
    printf("COLLISION between SRDF " TAGINT_FORMAT
    ...
    printf("COLLISION between SRDF " TAGINT_FORMAT " and WALL %d\n",
           atom->tag[i],j);
    printf("  bounce # = %d\n",ibounce+1);
    printf("  local indices: %d %d\n",i,j);
    printf("  timestep = %g\n",dt);
